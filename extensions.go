...
//     opaque bsk<1..2^16-1>;
// } BootstrapKey;
type BootstrapKeyExtensionCH struct {
        Bsk         []byte `tls:"head=2,min=1"`
}

func (c BootstrapKeyExtensionCH) Type() ExtensionType {
        return ExtensionTypeBootstrapKey
}

func (c BootstrapKeyExtensionCH) Marshal() ([]byte, error) {
        return syntax.Marshal(c)
}

func (c *BootstrapKeyExtensionCH) Unmarshal(data []byte) (int, error) {
        return syntax.Unmarshal(data, c)
}

// struct {
//     opaque KeyExchange<0..2^16-1>;
// } BootstrapKey;
type BootstrapKeyExtensionSH struct {
	KeyExchange []byte `tls:"head=2,min=0"`
}

func (c BootstrapKeyExtensionSH) Type() ExtensionType {
        return ExtensionTypeBootstrapKey
}

func (c BootstrapKeyExtensionSH) Marshal() ([]byte, error) {
        return syntax.Marshal(c)
}

func (c *BootstrapKeyExtensionSH) Unmarshal(data []byte) (int, error) {
        return syntax.Unmarshal(data, c)
}
